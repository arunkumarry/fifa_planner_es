...
function normalizeMonth(month: string | undefined): string | undefined {
  if (!month) return month;
  const m = month.trim().toLowerCase();
  if (m === 'june' || m === 'jun') return 'June';
  if (m === 'july' || m === 'jul') return 'July';
  // Fallback to title case
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === 'get_match_schedule') {
      const rawTeams = (args?.teams as string[]) || [];
      const teams = rawTeams.map(t => normalizeTeam(t) || t);
      const city = normalizeCity(args?.city as string);
    ...
    if (name === 'get_stadium_and_tickets') {
      const stadiumName = args?.stadium_name as string;
    ...
    if (name === 'find_nearby_accommodations') {
      ...
      const hotels = response.hits.hits.map((hit: any) => ({
        ...hit._source,
    ...
    if (name === 'predict_weather_conditions') {
      const city = normalizeCity(args?.city as string) || args?.city as string;
      const month = normalizeMonth(args?.month as string) || args?.month as string;
      ...
      try {
        ...
        const columns = response.columns?.map((c: any) => c.name) || [];
    ...
    if (name === 'calculate_hotel_eta') {
      const hLat = args?.hotel_lat as number;
      const hLon = args?.hotel_lon as number;
      const sLat = args?.stadium_lat as number;
      const sLon = args?.stadium_lon as number;
    ...
    if (name === 'get_team_historical_stats') {
      const normalizedTeam = normalizeTeam(args?.team as string) || args?.team as string;

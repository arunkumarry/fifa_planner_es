...
async function main() {
    ...
    try {
        ...
        const citiesRaw = await parseCsv(path.join(dataDir, 'host_cities.csv'));
        const teamsRaw = await parseCsv(path.join(dataDir, 'teams.csv'));
        const stagesRaw = await parseCsv(path.join(dataDir, 'tournament_stages.csv'));
        const matchesRaw = await parseCsv(path.join(dataDir, 'matches.csv'));
        const previousStatsRaw = await parseCsv(path.join(dataDir, 'previous_fifa_stats.csv'));
        const predictedStatsRaw = await parseCsv(path.join(dataDir, 'predicted_fifa_stats.csv'));
        ...
        if (accommodationsOps.length > 0) {
            ...
            const rows = generatedAccommodations.map(h => `"${h.hotel_id}","${h.name}","${h.stadium_id}",${h.latitude},${h.longitude},${h.price_per_night},${h.rating}`).join("\n");
        ...
        if (weatherOps.length > 0) {
            ...
            const rows = generatedWeather.map(w => `"${w.city}","${w.month}",${w.day},${w.avg_temp_f},${w.precipitation_chance},"${w.conditions}"`).join("\n");
        ...
        if (fs.existsSync(frontendSrcDir)) {
            ...
            fs.writeFileSync(path.join(frontendSrcDir, 'data.json'), JSON.stringify(dataExport, null, 2));
